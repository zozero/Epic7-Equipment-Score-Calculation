...
def 评估(模型, 设备):
    ...
    for 索引, (原图, 图片, 标签, 偏移量) in enumerate(测试用数据加载器):
        ...
        前景概率大于阈值的索引 = np.where(标签概率[0, :, 1] > 0.76)[0]
...
if __name__ == '__main__':
    ...
    # 训练用数据.__getitem__(5)
    训练用数据加载器 = DataLoader(训练用数据, batch_size=2, shuffle=False)

    设备 = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    模型 = 连接文本提取网络()
    if torch.cuda.device_count() > 1:
        print("让我们使用", torch.cuda.device_count(), "个图形处理单元吧!")
