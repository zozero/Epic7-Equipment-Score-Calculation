...
class 卷积重叠网络(nn.Module):
    ...
    def forward(self, 输入):
        ...
        # print('1：', x.size())
        x = self.池化2(self.整流线性函数2_2(self.卷积2_2(self.整流线性函数2_1(self.批归一化2(self.卷积2_1(x))))))
        # print('2：', x.size())
        x = self.池化3(self.整流线性函数3(self.批归一化3(self.卷积3(x))))
        # print('3：', x.size())
        卷积 = self.整流线性函数4(self.批归一化4(self.卷积4(x)))
        # print('4：', 卷积.size())
        # exit()
        卷积 = 卷积.squeeze(2)
        卷积 = 卷积.permute(2, 0, 1)
